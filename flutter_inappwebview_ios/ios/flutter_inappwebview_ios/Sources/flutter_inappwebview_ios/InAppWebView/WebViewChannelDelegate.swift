...
public class WebViewChannelDelegate: ChannelDelegate {
    ...
    public override func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case .setCloseButton:
            if let iabController = webView?.inAppBrowserDelegate as? InAppBrowserWebViewController {
                let msgMap = arguments as! Dictionary<String,String>
                iabController.showCloseButton(closeButtonOptions: msgMap)
            } else {
            }
            result(true)
            break
        case .updateWebViewLayout:
            if let iabController = webView?.inAppBrowserDelegate as? InAppBrowserWebViewController {
                let isLandscape = arguments?["isLandscape"] as? Bool ?? false
                iabController.updateWebViewLayout(isLandscape: isLandscape)
            }
            result(true)
            break
        case .setTransparent:
